...
import root.Gui.controller.MyController;
...
public class Graphics {
	...
	private static Node hubInsertNode(HUB hubObject, Pane canvas, ContextMenu contextMenu) {
		Image image = new Image("root/Gui/Images/HUB.png");
	...
	private static Node VMInsert(VM vmObject, Pane canvas, ContextMenu contextMenu) {
		//Selecting the Location of the image needed
				Image image = new Image("root/Gui/Images/VM.png");
				ImagePattern imagePattern = new ImagePattern(image);
	...
	private static Node createVlanNode(String nodeLabel, Pane canvas, ContextMenu contextMenu) {
		...
		//Selecting the Location of the image needed
				Image image = new Image("root/Gui/Images/ROUTER.png");
				ImagePattern imagePattern = new ImagePattern(image);
	...
	public static void draw(Pane canvas, ContextMenu contextMenu) {
		...
		for (Map.Entry<String, HUB> hubEntry : PatternConfig.hubMap.entrySet()) {
			...
			HUB currentHub = PatternConfig.hubMap.get(currentHubName);
			...
			for (Map.Entry<String, VM> vmEntry : PatternConfig.vmMap.entrySet()) {
				...
				VM currentVM = PatternConfig.vmMap.get(currentVMName);
				for (Map.Entry<String, String> vmInterface : currentVM.getInterfaces().entrySet()) {
				
					int ipClass = PatternConfig.getIPClass(PatternConfig.hubMap.get(currentHubName).getNetmask());
					String replaceRegex = "\\.\\d{1,"+ String.valueOf(ipClass) +"}\\z";
					if (vmInterface.getValue().replaceAll(replaceRegex, "").equals(
							PatternConfig.hubMap.get(currentHubName).getSubnet().replaceAll(replaceRegex, ""))) {
						PatternConfig.hubMap.get(currentHubName).addInf(currentVM.getName()+"."+vmInterface.getKey());
